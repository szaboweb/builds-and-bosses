...
void main() {
  ...
  group('Tactical Mode Game State & Planning Loop Tests', () {
    ...
    setUp(() async {
      ...
      game.overlays.addEntry('actionBar', (context, game) => const SizedBox.shrink());
      game.overlays.addEntry('planningHud', (context, game) => const SizedBox.shrink());
    ...
    test('queueActionAt queues selected action within budget', () {
      ...
      // Change action type to Move (15 AP)
      ...
      expect(game.actionQueue.actions.last, isA<MoveAction>());
      expect(game.actionQueue.spentAP, equals(40));
    ...
    test('Space key executes plan during planning mode', () {
      game.startPlanning();
      game.queueActionAt(Vector2(100, 100));

      // Press Space in planning -> Executes plan
    ...
    test('Player platformer physics lands on ground after falling', () {
      ...
      expect(game.player.isOnGround, isTrue);
      expect(game.player.position.y + game.player.size.y / 2, closeTo(game.arena.groundY, 0.1));
    });
    ...
    test('Player can land on elevated platform', () {
      ...
      expect(game.player.isOnGround, isTrue);
      expect(game.player.position.y + game.player.size.y / 2, closeTo(plat.top, 0.1));
    });
  });
}
