...
enum LogLevel { debug, info, warn, error, combat }
...
class CombatLogger {
  ...
  void logCombatAttack({
    ...
    final message =
        ...
        'Attack Roll: ${result.rawD20Roll} '
        '+ ${attacker.strengthMod} (STR) '
        '+ ${attacker.proficiencyBonus} (Proficiency) '
        '= ${result.totalAttackRoll} vs AC ${result.targetAC} '
        '-> $rollType! Damage: ${result.damageDealt} '
    ...
    log(
      ...
      data: {
        ...
        'strengthMod': attacker.strengthMod,
  ...
  void logCombatSpell({
    ...
    final ability = attacker.classId == 'wizard'
        ? 'INT'
        : attacker.classId == 'cleric'
        ? 'WIS'
        : 'CHA';
    final outcome = result.isCritical
        ? 'CRITICAL HIT'
        : result.isHit
        ? 'HIT'
    ...
    log(
      ...
      category: 'SPELL',
      message:
          'Spell Attack: ${result.rawD20Roll} + ${attacker.spellcastingAbilityMod} '
          '($ability) + ${attacker.proficiencyBonus} (Proficiency) = '
          '${result.totalAttackRoll} vs AC ${result.targetAC} -> $outcome! '
          'Damage: ${result.damageDealt}',
      data: {
        'spellcaster': attacker.name,
        'spellAbility': ability,
        'rawD20': result.rawD20Roll,
        'spellcastingModifier': attacker.spellcastingAbilityMod,
        'proficiencyBonus': attacker.proficiencyBonus,
        'totalRoll': result.totalAttackRoll,
        'targetAC': result.targetAC,
        'isHit': result.isHit,
  ...
  void logCombatRanged({
    ...
    log(
      ...
      category: 'RANGED',
      message:
          'Ranged Attack: ${result.rawD20Roll} + ${attacker.dexterityMod} '
          '(DEX) + ${attacker.proficiencyBonus} (Proficiency) = '
          '${result.totalAttackRoll} vs AC ${result.targetAC} -> '
          '${result.description}${disadvantage ? ' [DISADVANTAGE]' : ''} '
          'Damage: ${result.damageDealt}',
      data: {
        'attacker': attacker.name,
        'defender': defender.name,
        'rawD20': result.rawD20Roll,
        'dexterityMod': attacker.dexterityMod,
        'proficiencyBonus': attacker.proficiencyBonus,
        'totalRoll': result.totalAttackRoll,
        'targetAC': result.targetAC,
        'isHit': result.isHit,
  ...
  void logTacticalAction({
    ...
    final message =
        '[$eventType] ${action.name} (${action.apCost} AP) -> '
  ...
  void logWarning(
    ...
    log(level: LogLevel.warn, category: category, message: message, data: data);
  }
  ...
  void logBuildChange({
    ...
    final message =
        'Build applied for ${newStats.name}: '
