...
* A játéklogika nem használhat Windows- vagy Linux-specifikus API-t. Platformfüggő műveletekhez a Python standard könyvtárát (`pathlib`, `platform`, `appdirs`-szerű megoldás) használjuk.
* Fájlutakat mindig `pathlib.Path` segítségével építünk fel; tilos a kézzel összefűzött `\\` vagy `/` útvonal.
...
* A játékadatok JSON-ban maradnak, a Python kód pedig a szabályokat és a végrehajtást tartalmazza. A tartalom hozzáadása lehetőleg kódmódosítás nélkül történjen.
...
* **Kis, tesztelhető egységek:** a sebzésszámítás, mentődobások, debuffok, loot és időmérés pygame ablak nélkül is tesztelhető.
...
A Campaign Blueprint a futam egyetlen forrása a hős szintjéhez, a multiclass elosztáshoz, az attribútumokhoz, a feat-ekhez és az enhancement pontokhoz. Egy blueprint legfeljebb három különböző kasztot tartalmazhat. A `src/core/campaign.py` pygame-független domain-modellje validálja a választásokat, majd ugyanebből hozza létre a meglévő `Character` harci modellt.
