"""
Dungeon run state — prototype version with one room and one boss.

Layout:
    - Player and boss spawn on the same side-view floor
    - A small platform layout provides the first platformer-shaped arena
    - Tiled map loading remains a follow-up step

Controls:
    A/D or left/right  — move
    W/up               — jump
  SPACE              — basic attack (melee or ranged depending on primary class)
  ESC                — abandon run -> main menu

TODO: add tile-based room rendering
TODO: add trap zones (attrition / curse system)
TODO: add multiple rooms and mini-bosses
TODO: add ability bar (1-6 keys)
TODO: add loot drop on boss kill
"""

import math
import time
import pygame
from dataclasses import dataclass
from typing import Optional, Dict, Any

from src.core.state_machine import State, StateMachine
from src.core.constants import (
    SCREEN_WIDTH, SCREEN_HEIGHT,
    PIXEL_TILE_SIZE,
    COLOR_BG_DARK, COLOR_ACCENT_GOLD, COLOR_ACCENT_RED,
    COLOR_ACCENT_BLUE, COLOR_ACCENT_GREEN,
    COLOR_TEXT_LIGHT, COLOR_TEXT_MUTED, COLOR_PANEL_BORDER,
    GameStateId,
)
from src.core.data_loader import DataLoader
from src.core.campaign import CampaignProgression
from src.core.sprite_loader import SpriteLoader
from src.core.animation import AnimationClip, SpriteAnimator
from src.combat.combat_resolver import resolve_player_attack, resolve_boss_attack, AttackResult
from src.entities.character import Character


# ---------------------------------------------------------------------------
# Boss entity — fully data-driven from data/bosses/*.json
# ---------------------------------------------------------------------------

class Boss:
    """Boss entity built from a JSON stat-block (schema boss/v1)."""

    @classmethod
    def from_data(cls, data: dict) -> "Boss":
        boss = cls.__new__(cls)
        s = data["stats"]
        spawn = data.get("spawn", {})

        boss.id               = data["id"]
        boss.name             = data["name"]
        boss.icon             = data.get("icon", "\U0001F480")
        boss.color            = tuple(data.get("color", [140, 60, 200]))
        boss.radius           = data.get("radius", 32)
        boss.projectile       = data.get("projectile")

        boss.max_hp           = s["max_hp"]
        boss.current_hp       = s["max_hp"]
        boss.armor_class      = s["armor_class"]
        boss.base_speed       = s["speed"]
        boss.speed            = s["speed"]
        boss.base_attack_rate = s["attack_rate"]
        boss.attack_rate      = s["attack_rate"]
        boss.attack_bonus     = s["attack_bonus"]
        boss.damage_die       = s["damage_die"]
        boss.damage_count     = s["damage_count"]

        boss.phases = sorted(
            data.get("phases", []),
            key=lambda p: p["hp_threshold_pct"]
        )

        boss.x = SCREEN_WIDTH * spawn.get("x_pct", 0.5)
        boss.y = float(spawn.get("y_px", ARENA_BOTTOM - boss.radius))
        boss.velocity_y = 0.0
        boss.grounded = False

        boss.attack_cooldown = 0.0
        boss.alive = True
        return boss

    @property
    def hp_ratio(self) -> float:
        return self.current_hp / max(self.max_hp, 1)

    def take_damage(self, amount: int) -> None:
        self.current_hp = max(0, self.current_hp - amount)
        if self.current_hp == 0:
            self.alive = False
        self._update_phase()

    def _update_phase(self) -> None:
        for phase in self.phases:
            if self.hp_ratio <= phase["hp_threshold_pct"]:
                self.speed = self.base_speed * phase.get("speed_multiplier", 1.0)
                self.attack_rate = self.base_attack_rate * phase.get("attack_rate_multiplier", 1.0)

    def update(self, dt: float, px: float, py: float) -> Optional[str]:
        """Move toward player; return 'pending' when an attack is due."""
        if not self.alive:
            return None
        if not self.grounded:
            self.velocity_y += GRAVITY * dt
            previous_bottom = self.y + self.radius
            self.y += self.velocity_y * dt
            if previous_bottom <= GROUND_Y and self.y + self.radius >= GROUND_Y:
                self.y = GROUND_Y - self.radius
                self.velocity_y = 0.0
                self.grounded = True
            return None
        dx = px - self.x
        horizontal_distance = abs(dx)
        if horizontal_distance > self.radius + 28:
            self.x += (1 if dx > 0 else -1) * self.speed * dt
        dist = horizontal_distance
        self.attack_cooldown -= dt
        if self.attack_cooldown <= 0:
            self.attack_cooldown = self.attack_rate
            if dist < self.radius + 28 + 10:
                return "pending"
            if self.projectile:
                return "ranged"
        return None


@dataclass
class BossProjectile:
    """A data-driven boss projectile travelling toward the player."""
    x: float
    y: float
    vx: float
    vy: float
    speed: float
    radius: int
    damage_die: int
    damage_count: int
    color: tuple[int, int, int]
    kind: str
    life: float = 5.0


# ---------------------------------------------------------------------------
# Dungeon Run State
# ---------------------------------------------------------------------------

ARENA_LEFT = 80
ARENA_RIGHT = SCREEN_WIDTH - 80
ARENA_BOTTOM = SCREEN_HEIGHT - 120
GROUND_Y = ARENA_BOTTOM
GRAVITY = 1500.0
JUMP_SPEED = -560.0
PLATFORM_RECTS = (
    pygame.Rect(180, ARENA_BOTTOM - 72, 220, 18),
    pygame.Rect(820, ARENA_BOTTOM - 104, 240, 18),
)

PLAYER_RADIUS = PIXEL_TILE_SIZE // 2
PLAYER_DRAW_SIZE = 80
PLAYER_DRAW_RADIUS = PLAYER_DRAW_SIZE // 2
PLAYER_COLOR = (80, 180, 255)

# Boss roster is loaded at runtime from data/bosses/*.json via DataLoader
# To add a boss: create data/bosses/boss_00N_name.json — no code change needed.
BOSS_ORDER = [
    "boss_001_malakar",
    # "boss_002_ignis",     # TODO
    # "boss_003_serytha",   # TODO
]

# Weapon icon per primary class
CLASS_WEAPON_ICON: dict[str, str] = {
    "fighter":  "\U00002694",   # crossed swords
    "paladin":  "\U0001F531",   # trident / holy symbol
    "rogue":    "\U0001F462",   # boot (thief)
    "wizard":   "\U0001FA84",   # magic wand
    "cleric":   "\U0001F6F0",   # staff / satellite (placeholder)
    "ranger":   "\U0001F3F9",   # bow
    "barbarian":"\U0001FA93",   # axe
    "bard":     "\U0001F3B5",   # music note
}
DEFAULT_WEAPON_ICON = "\U00002694"  # crossed swords fallback


class DungeonRunState(State):
    """Top-level state for a single dungeon run (prototype: one room, one boss)."""

    def __init__(self, state_machine: StateMachine):
        super().__init__(state_machine)
        self.character: Optional[Character] = None
        self.campaign: Optional[CampaignProgression] = None
        self.boss_number: int = 1
        self.boss_level: int = 0
        self.boss: Optional[Boss] = None

        # Player runtime position and HP
        self.px: float = ARENA_LEFT + PLAYER_RADIUS + 24
        self.py: float = ARENA_BOTTOM - 40
        self.velocity_y: float = 0.0
        self.grounded: bool = True
        self.facing: int = 1
        self.current_hp: int = 0
        self.current_mana: int = 0

        # Player attack
        self.attack_cooldown: float = 0.0
        self.attack_range: int = 200           # prototype: ranged basic attack
        self.attack_damage: int = 0
        self.projectiles: list[BossProjectile] = []
        self.block_timer: float = 0.0
        self.blocked_count: int = 0
        self.last_defense: str = ""

        # Run timer
        self.run_start: float = 0.0
        self.run_time: float = 0.0
        self.state: str = "running"            # "running" | "victory" | "dead"
        self.result_timer: float = 3.0         # seconds before returning to menu

        # Fonts
        self.font_hud: Optional[pygame.font.Font] = None
        self.font_big: Optional[pygame.font.Font] = None
        self.font_small: Optional[pygame.font.Font] = None
        self.font_icon_large: Optional[pygame.font.Font] = None
        self.font_icon_small: Optional[pygame.font.Font] = None
        self._weapon_icon: str = DEFAULT_WEAPON_ICON

        # Attack flash effect
        self.hit_flash: float = 0.0
        self.player_anim_time: float = 0.0
        self.player_moving: bool = False
        self.player_animator: Optional[SpriteAnimator] = None
        self.player_animator_class: str = ""

    # ------------------------------------------------------------------
    # Lifecycle
    # ------------------------------------------------------------------

    def enter(self, enter_data: Optional[Dict[str, Any]] = None) -> None:
        enter_data = enter_data or {}
        self.character = enter_data.get("character")
        self.campaign = enter_data.get("campaign", self.engine.campaign)
        self.boss_number = self.campaign.boss_number

        if self.character is None:
            # Fallback: create a default character for testing
            from src.entities.character import Character
            self.character = Character("TestHero")
            self.character.add_class("fighter", 3)
            self.character.add_class("rogue", 2)
            self.character.build()

        self.current_hp = self.character.stats.max_hp
        self.current_mana = self.character.stats.max_mana
        # attack_damage is now derived per-roll in combat_resolver; keep as fallback min
        self.attack_damage = 1  # unused directly — resolver handles it

        # Spawn player
        self.px = ARENA_LEFT + PLAYER_RADIUS + 24
        self.py = GROUND_Y - PLAYER_RADIUS
        self.velocity_y = 0.0
        self.grounded = True
        self.facing = 1

        # Spawn boss from JSON
        boss_index = min(self.boss_number - 1, len(BOSS_ORDER) - 1)
        boss_data = DataLoader.boss(BOSS_ORDER[boss_index])
        self.boss = Boss.from_data(boss_data)
        self.boss_level = self.campaign.scaling.boss_level
        if self.campaign.phase.value == "gauntlet":
            self.campaign.start_boss()

        self.run_start = time.monotonic()
        self.run_time = 0.0
        self.state = "running"
        self.result_timer = 3.0
        self.attack_cooldown = 0.0
        self.projectiles = []
        self.block_timer = 0.0
        self.blocked_count = 0
        self.last_defense = ""
        self.player_anim_time = 0.0
        self.player_moving = False
        self.player_animator = None
        self.player_animator_class = ""
        self.hit_flash = 0.0
        self._last_player_attack: Optional[AttackResult] = None
        self._last_boss_attack: Optional[AttackResult] = None
        self._weapon_icon: str = self._resolve_weapon_icon()
        self._init_fonts()

    def _resolve_weapon_icon(self) -> str:
        """Pick weapon icon based on the highest-level class."""
        if not self.character or not self.character.class_levels:
            return DEFAULT_WEAPON_ICON
        primary = max(self.character.class_levels, key=lambda c: self.character.class_levels[c])
        return CLASS_WEAPON_ICON.get(primary, DEFAULT_WEAPON_ICON)

    def _init_fonts(self) -> None:
        if self.font_hud is None and pygame.font.get_init():
            self.font_hud = pygame.font.SysFont("consolas,monospace", 22, bold=True)
            self.font_big = pygame.font.SysFont("consolas,monospace", 48, bold=True)
            self.font_small = pygame.font.SysFont("consolas,monospace", 18)
            # Emoji font — optional; None is safe (sprite PNG used instead)
            import warnings
            with warnings.catch_warnings():
                warnings.simplefilter("ignore", UserWarning)
                self.font_icon_large = pygame.font.SysFont("segoeuiemoji,notocoloremoji,unifont", 52)
                self.font_icon_small = pygame.font.SysFont("segoeuiemoji,notocoloremoji,unifont", 26)

    # ------------------------------------------------------------------
    # Events
    # ------------------------------------------------------------------

    def handle_event(self, event: pygame.event.Event) -> None:
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_ESCAPE:
                self.state_machine.change_state(GameStateId.MAIN_MENU)
            elif event.key in (pygame.K_w, pygame.K_UP) and self.grounded and self.state == "running":
                self.velocity_y = JUMP_SPEED
                self.grounded = False
            elif event.key == pygame.K_SPACE and self.state == "running":
                self._try_space_action()

    # ------------------------------------------------------------------
    # Update
    # ------------------------------------------------------------------

    def update(self, dt: float) -> None:
        if self.state != "running":
            self.result_timer -= dt
            if self.result_timer <= 0:
                self.state_machine.change_state(
                    GameStateId.SCOREBOARD,
                    {
                        "character": self.character,
                        "run_time": self.run_time,
                        "victory": self.state == "victory",
                    }
                )
            return

        self.run_time = time.monotonic() - self.run_start
        self.attack_cooldown = max(0.0, self.attack_cooldown - dt)
        self.block_timer = max(0.0, self.block_timer - dt)
        self.hit_flash = max(0.0, self.hit_flash - dt)

        self.player_moving = self._move_player(dt)
        self.player_anim_time += dt if self.player_moving else dt * 0.35
        self._update_player_animation(dt)

        if self.boss and self.boss.alive:
            signal = self.boss.update(dt, self.px, self.py)
            if signal == "pending":
                result = resolve_boss_attack(
                    self.boss.attack_bonus,
                    self.boss.damage_die,
                    self.boss.damage_count,
                    self.character.stats.armor_class,
                )
                self._last_boss_attack = result
                if result.hit:
                    self.current_hp -= result.damage
                    self.hit_flash = 0.3
                    if self.current_hp <= 0:
                        self.current_hp = 0
                        self.state = "dead"
                        self.result_timer = 3.5
            elif signal == "ranged":
                self._spawn_boss_projectile()

        self._update_projectiles(dt)

        if self.boss and not self.boss.alive and self.state == "running":
            self.state = "victory"
            self.result_timer = 4.0
            if self.campaign and self.campaign.phase.value == "boss":
                self.campaign.complete_boss()

    def _move_player(self, dt: float) -> bool:
        keys = pygame.key.get_pressed()
        spd = self.character.stats.move_speed * dt
        dx = 0
        if keys[pygame.K_LEFT] or keys[pygame.K_a]:
            dx -= 1
        if keys[pygame.K_RIGHT] or keys[pygame.K_d]:
            dx += 1

        moving = bool(dx)
        if dx:
            self.facing = 1 if dx > 0 else -1

        self.px = max(ARENA_LEFT + PLAYER_RADIUS, min(ARENA_RIGHT - PLAYER_RADIUS, self.px + dx * spd))
        previous_bottom = self.py + PLAYER_RADIUS
        self.velocity_y += GRAVITY * dt
        self.py += self.velocity_y * dt
        self.grounded = False

        if self.velocity_y >= 0 and previous_bottom <= GROUND_Y and self.py + PLAYER_RADIUS >= GROUND_Y:
            self.py = GROUND_Y - PLAYER_RADIUS
            self.velocity_y = 0.0
            self.grounded = True
        else:
            for platform in PLATFORM_RECTS:
                if (self.velocity_y >= 0 and previous_bottom <= platform.top
                        and self.py + PLAYER_RADIUS >= platform.top
                        and platform.left - PLAYER_RADIUS <= self.px <= platform.right + PLAYER_RADIUS):
                    self.py = platform.top - PLAYER_RADIUS
                    self.velocity_y = 0.0
                    self.grounded = True
                    break
        return moving

    def _update_player_animation(self, dt: float) -> None:
        """Advance a spritesheet animation, with a static-sprite fallback."""
        primary = max(self.character.class_levels,
                      key=lambda c: self.character.class_levels[c]) \
            if self.character.class_levels else "fighter"
        if self.player_animator is None or self.player_animator_class != primary:
            clips: dict[str, AnimationClip] = {}
            for state, frame_count, fps in (
                ("idle", 4, 6.0),
                ("run", 6, 10.0),
                ("jump", 4, 10.0),
                ("fall", 2, 8.0),
                ("attack", 6, 12.0),
                ("block", 4, 10.0),
                ("hit", 2, 8.0),
                ("death", 6, 8.0),
            ):
                frames = SpriteLoader.animation_frames(
                    f"characters/{primary}/{state}.png",
                    frame_count=frame_count,
                )
                if frames:
                    clips[state] = AnimationClip(frames, fps=fps, loop=state not in {"hit", "death"})
            fallback = SpriteLoader.character(primary, size=PLAYER_DRAW_SIZE)
            if fallback is not None:
                clips.setdefault("idle", AnimationClip([fallback], fps=1.0))
                for state in ("run", "jump", "fall", "attack", "block", "hit", "death"):
                    clips.setdefault(state, clips["idle"])
            self.player_animator = SpriteAnimator(clips)
            self.player_animator_class = primary
        if not self.grounded:
            animation_state = "jump" if self.velocity_y < 0 else "fall"
        else:
            animation_state = "run" if self.player_moving else "idle"
        self.player_animator.set_state(animation_state)
        self.player_animator.update(dt)

    def _try_attack(self) -> None:
        if self.attack_cooldown > 0 or not self.boss or not self.boss.alive:
            return
        dist = math.hypot(self.boss.x - self.px, self.boss.y - self.py)
        if dist <= self.attack_range:
            result = resolve_player_attack(self.character, self.boss.armor_class)
            self._last_player_attack = result
            if result.hit:
                self.boss.take_damage(result.damage)
            self.attack_cooldown = 0.6

    def _try_space_action(self) -> None:
        """Attack normally; use SPACE as a timed defense near a projectile."""
        incoming = self._nearest_projectile_distance()
        if incoming is not None and incoming <= 85:
            self.block_timer = 0.28
            return
        self._try_attack()

    def _nearest_projectile_distance(self) -> Optional[float]:
        if not self.projectiles:
            return None
        return min(math.hypot(p.x - self.px, p.y - self.py) for p in self.projectiles)

    def _spawn_boss_projectile(self) -> None:
        if not self.boss or not self.boss.projectile:
            return
        spec = self.boss.projectile
        dx = self.px - self.boss.x
        distance = max(abs(dx), 1.0)
        self.projectiles.append(BossProjectile(
            x=self.boss.x,
            y=self.boss.y,
            vx=(1 if dx > 0 else -1),
            vy=0.0,
            speed=float(spec.get("speed", 220)),
            radius=int(spec.get("radius", 8)),
            damage_die=int(spec.get("damage_die", self.boss.damage_die)),
            damage_count=int(spec.get("damage_count", 1)),
            color=tuple(spec.get("color", self.boss.color)),
            kind=str(spec.get("kind", "boss_projectile")),
        ))

    def _update_projectiles(self, dt: float) -> None:
        remaining: list[BossProjectile] = []
        for projectile in self.projectiles:
            projectile.x += projectile.vx * projectile.speed * dt
            projectile.y += projectile.vy * projectile.speed * dt
            projectile.life -= dt
            distance = math.hypot(projectile.x - self.px, projectile.y - self.py)
            if distance <= projectile.radius + PLAYER_RADIUS:
                if self.block_timer > 0:
                    self._defend_projectile(projectile)
                else:
                    damage = resolve_boss_attack(
                        self.boss.attack_bonus,
                        projectile.damage_die,
                        projectile.damage_count,
                        self.character.stats.armor_class,
                    ).damage
                    self.current_hp = max(0, self.current_hp - damage)
                    self.hit_flash = 0.3
                    if self.current_hp <= 0:
                        self.state = "dead"
                        self.result_timer = 3.5
                continue
            if projectile.life > 0:
                remaining.append(projectile)
        self.projectiles = remaining

    def _defend_projectile(self, projectile: BossProjectile) -> None:
        primary = max(self.character.class_levels, key=self.character.class_levels.get) \
            if self.character.class_levels else "fighter"
        defense_names = {
            "fighter": "PARRY",
            "paladin": "PARRY",
            "barbarian": "PARRY",
            "rogue": "DODGE",
            "ranger": "DODGE",
            "wizard": "BARRIER",
            "sorcerer": "BARRIER",
            "cleric": "BARRIER",
            "bard": "WARD",
        }
        self.last_defense = defense_names.get(primary, "BLOCK")
        self.blocked_count += 1

    # ------------------------------------------------------------------
    # Render
    # ------------------------------------------------------------------

    def render(self, surface: pygame.Surface) -> None:
        if not self.font_hud:
            self._init_fonts()

        # Side-view dungeon backdrop and platform geometry.
        surface.fill((13, 16, 25))
        pygame.draw.rect(surface, (24, 28, 43), (0, 70, SCREEN_WIDTH, ARENA_BOTTOM - 70))
        pygame.draw.rect(surface, (34, 39, 57), (0, ARENA_BOTTOM, SCREEN_WIDTH, SCREEN_HEIGHT - ARENA_BOTTOM))
        pygame.draw.line(surface, (93, 99, 124), (0, ARENA_BOTTOM), (SCREEN_WIDTH, ARENA_BOTTOM), 3)
        for platform in PLATFORM_RECTS:
            pygame.draw.rect(surface, (46, 52, 74), platform)
            pygame.draw.line(surface, COLOR_ACCENT_GOLD,
                             (platform.left, platform.top), (platform.right, platform.top), 2)

        # Boss
        if self.boss and self.boss.alive:
            boss_bob = math.sin(self.run_time * 7.0)
            boss_draw_y = int(self.boss.y + boss_bob * 4)
            boss_shadow_w = int(self.boss.radius * (1.0 - boss_bob * 0.08))
            pygame.draw.ellipse(
                surface,
                (8, 9, 15),
                (int(self.boss.x) - boss_shadow_w, int(self.boss.y) + self.boss.radius - 4,
                 boss_shadow_w * 2, 10),
            )
            boss_sprite = SpriteLoader.boss(self.boss.id, size=self.boss.radius * 2)
            self._draw_entity_icon(
                surface, self.boss.icon, int(self.boss.x), boss_draw_y,
                self.boss.radius, self.boss.color, large=True, sprite=boss_sprite,
            )
            self._draw_hp_bar(surface, self.boss.x - 40, boss_draw_y - self.boss.radius - 14,
                              80, 8, self.boss.hp_ratio, COLOR_ACCENT_RED)
            name_surf = self.font_small.render(self.boss.name, True, COLOR_TEXT_MUTED)
            surface.blit(name_surf, (int(self.boss.x) - name_surf.get_width() // 2,
                                     boss_draw_y - self.boss.radius - 30))

        # Boss projectiles
        for projectile in self.projectiles:
            center = (int(projectile.x), int(projectile.y))
            pygame.draw.circle(surface, projectile.color, center, projectile.radius)
            pygame.draw.circle(surface, COLOR_TEXT_LIGHT, center, projectile.radius + 3, 1)

        # Player body
        primary = max(self.character.class_levels,
                      key=lambda c: self.character.class_levels[c]) \
            if self.character.class_levels else "fighter"
        player_sprite = (
            self.player_animator.frame()
            if self.player_animator is not None
            else SpriteLoader.character(primary, size=PLAYER_DRAW_SIZE)
        )
        if player_sprite is not None and player_sprite.get_size() != (PLAYER_DRAW_SIZE, PLAYER_DRAW_SIZE):
            player_sprite = pygame.transform.scale(player_sprite, (PLAYER_DRAW_SIZE, PLAYER_DRAW_SIZE))
        if player_sprite is not None and self.facing < 0:
            player_sprite = pygame.transform.flip(player_sprite, True, False)
        p_color = (255, 80, 80) if self.hit_flash > 0 else PLAYER_COLOR
        player_draw_y = int(self.py)
        if self.player_moving:
            player_bob = math.sin(self.player_anim_time * 18.0)
            player_draw_y += int(player_bob * 4)
        else:
            player_bob = math.sin(self.player_anim_time * 4.0)
            player_draw_y += int(player_bob * 2)
        player_shadow_w = int(PLAYER_DRAW_RADIUS * 0.8 * (1.0 - player_bob * 0.1))
        pygame.draw.ellipse(
            surface,
            (8, 9, 15),
            (int(self.px) - player_shadow_w, int(self.py) + PLAYER_RADIUS - 4,
             player_shadow_w * 2, 8),
        )
        if player_sprite is not None:
            if self.hit_flash > 0:
                # Red tint on hit
                tinted = player_sprite.copy()
                tinted.fill((255, 80, 80, 100), special_flags=pygame.BLEND_RGBA_MULT)
                surface.blit(tinted, (int(self.px) - PLAYER_DRAW_RADIUS,
                                      player_draw_y - PLAYER_DRAW_RADIUS))
            else:
                surface.blit(player_sprite, (int(self.px) - PLAYER_DRAW_RADIUS,
                                             player_draw_y - PLAYER_DRAW_RADIUS))
        else:
            pygame.draw.circle(surface, p_color, (int(self.px), player_draw_y), PLAYER_RADIUS)
            self._draw_weapon_icon(surface, int(self.px), player_draw_y)

        # Compact melee reach indicator, aligned with the character's facing.
        if self.attack_cooldown <= 0 and self.state == "running":
            attack_x = int(self.px + self.facing * (PLAYER_RADIUS + 28))
            pygame.draw.line(surface, (60, 90, 120),
                             (int(self.px), int(self.py)), (attack_x, int(self.py)), 2)

        self._draw_hud(surface)
        self._draw_overlay(surface)

    def _draw_entity_icon(
        self, surface: pygame.Surface, icon: str,
        cx: int, cy: int, radius: int, glow_color, large: bool = True,
        sprite: Optional[pygame.Surface] = None,
    ) -> None:
        """Render entity: sprite PNG if available, else glow circle + emoji fallback."""
        if sprite is not None:
            # Draw sprite centered, no glow needed
            surface.blit(sprite, (cx - sprite.get_width() // 2,
                                   cy - sprite.get_height() // 2))
            return

        # Fallback: glow circle + emoji
        pygame.draw.circle(surface, glow_color, (cx, cy), radius)
        font = self.font_icon_large if large else self.font_icon_small
        if font:
            try:
                icon_surf = font.render(icon, True, (255, 255, 255))
                surface.blit(icon_surf, (cx - icon_surf.get_width() // 2,
                                         cy - icon_surf.get_height() // 2))
            except Exception:
                pass

    def _draw_weapon_icon(self, surface: pygame.Surface, px: int, py: int) -> None:
        """Draw weapon icon or sprite to the right of the player token."""
        # Try sprite first
        primary = max(self.character.class_levels,
                      key=lambda c: self.character.class_levels[c]) \
            if self.character.class_levels else "fighter"
        weapon_sprite = SpriteLoader.character(primary, size=int(PLAYER_RADIUS * 2))
        if weapon_sprite is not None:
            return  # player sprite already drawn in _draw_entity_icon
        # Emoji fallback
        if not self.font_icon_small:
            return
        try:
            w_surf = self.font_icon_small.render(self._weapon_icon, True, COLOR_ACCENT_GOLD)
            surface.blit(w_surf, (px + PLAYER_RADIUS + 4, py - w_surf.get_height() // 2))
        except Exception:
            pass

    def _draw_hp_bar(self, surface, x, y, w, h, ratio, color) -> None:
        pygame.draw.rect(surface, (50, 50, 50), (x, y, w, h))
        pygame.draw.rect(surface, color, (x, y, int(w * ratio), h))
        pygame.draw.rect(surface, COLOR_PANEL_BORDER, (x, y, w, h), 1)

    def _draw_hud(self, surface: pygame.Surface) -> None:
        # Player HP bar
        hp_ratio = self.current_hp / max(self.character.stats.max_hp, 1)
        self._draw_hp_bar(surface, 20, SCREEN_HEIGHT - 90, 220, 18, hp_ratio, COLOR_ACCENT_RED)
        hp_txt = self.font_hud.render(
            f"HP  {self.current_hp} / {self.character.stats.max_hp}", True, COLOR_TEXT_LIGHT)
        surface.blit(hp_txt, (20, SCREEN_HEIGHT - 114))

        # Mana bar
        mana_ratio = self.current_mana / max(self.character.stats.max_mana, 1)
        self._draw_hp_bar(surface, 20, SCREEN_HEIGHT - 60, 220, 14, mana_ratio, COLOR_ACCENT_BLUE)
        mana_txt = self.font_small.render(
            f"Mana  {self.current_mana} / {self.character.stats.max_mana}", True, COLOR_TEXT_MUTED)
        surface.blit(mana_txt, (20, SCREEN_HEIGHT - 78))

        # Timer
        mins = int(self.run_time) // 60
        secs = int(self.run_time) % 60
        timer_surf = self.font_hud.render(f"{mins:02d}:{secs:02d}", True, COLOR_ACCENT_GOLD)
        surface.blit(timer_surf, (SCREEN_WIDTH // 2 - timer_surf.get_width() // 2, 20))

        # Character info
        build_str = " / ".join(f"{cid.capitalize()} {lvl}" for cid, lvl in self.character.class_levels.items())
        build_surf = self.font_small.render(
            f"{self.character.name}  [{build_str}]", True, COLOR_TEXT_MUTED)
        surface.blit(build_surf, (SCREEN_WIDTH - build_surf.get_width() - 20, 20))

        level_surf = self.font_small.render(
            f"Hero Lv {self.character.total_level}  |  Boss Lv {self.boss_level}",
            True,
            COLOR_ACCENT_RED,
        )
        surface.blit(level_surf, (SCREEN_WIDTH - level_surf.get_width() - 20, 44))

        # Attack and defense hint
        if self.attack_cooldown > 0:
            cd_surf = self.font_small.render(
                f"Tamadas: {self.attack_cooldown:.1f}s", True, COLOR_TEXT_MUTED)
        else:
            cd_surf = self.font_small.render("SPACE = Tamadas / Vedekezes", True, COLOR_ACCENT_GREEN)
        surface.blit(cd_surf, (SCREEN_WIDTH - cd_surf.get_width() - 20, SCREEN_HEIGHT - 50))

        if self.last_defense:
            defense_surf = self.font_hud.render(
                self.last_defense, True, COLOR_ACCENT_GREEN
            )
            surface.blit(defense_surf, (SCREEN_WIDTH // 2 - defense_surf.get_width() // 2, 58))

    def _draw_overlay(self, surface: pygame.Surface) -> None:
        if self.state == "running":
            return

        overlay = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT), pygame.SRCALPHA)
        overlay.fill((0, 0, 0, 160))
        surface.blit(overlay, (0, 0))

        if self.state == "victory":
            msg = "GYOZELEM!"
            color = COLOR_ACCENT_GOLD
            mins = int(self.run_time) // 60
            secs = int(self.run_time) % 60
            sub = f"Ido: {mins:02d}:{secs:02d}   Pontszam betoltodik..."
        else:
            msg = "HALAL"
            color = COLOR_ACCENT_RED
            sub = "Jobb lesz legkozelebbi alkalommal..."

        big = self.font_big.render(msg, True, color)
        surface.blit(big, (SCREEN_WIDTH // 2 - big.get_width() // 2, SCREEN_HEIGHT // 2 - 50))
        small = self.font_small.render(sub, True, COLOR_TEXT_MUTED)
        surface.blit(small, (SCREEN_WIDTH // 2 - small.get_width() // 2, SCREEN_HEIGHT // 2 + 20))
